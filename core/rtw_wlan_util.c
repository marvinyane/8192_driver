...
int MirrorLink_info_handler(_adapter *padapter, PNDIS_802_11_VARIABLE_IEs	pIE)
{
	...
	if ( rtw_get_mirror_ie( ( u8* ) pIE, pIE->Length, wfd_ie, &wfd_ielen ) )
    {
        printk( "[%s] Found Mirror IE\n", __FUNCTION__ );
        pwdinfo->wfd_info->peer_mirror_ctrlport = rtw_get_mirror_port(wfd_ie, wfd_ielen);
        DBG_8192C( "[%s] Peer PORT NUM = %d\n", __FUNCTION__, pwdinfo->wfd_info->peer_mirror_ctrlport );
        return( _TRUE );
    }
	else
	{
		printk( "[%s] NO MirrorLink IE\n", __FUNCTION__ );
