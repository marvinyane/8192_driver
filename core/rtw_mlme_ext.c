...
unsigned char	ML_OUI[] = {0x04,0xDF,0x69,0x0A};
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	{		
		...
		if(rtw_get_mirror_ie(pframe + WLAN_HDR_A3_LEN + ie_offset, pkt_len - WLAN_HDR_A3_LEN - ie_offset , wfd_ie, &wfd_ielen )) 
        {
            DBG_8192C( "yjz MirrorLink IE found!!!!!!!!!!!!!\n");
            printk( "yjz MirrorLink IE found!!!!!!!!!!!!!\n");
        }
	...
	for (i = (6 + WLAN_HDR_A3_LEN); i < pkt_len;)
	{
		...
		switch (pIE->ElementID)
		{
			case _VENDOR_SPECIFIC_IE_:
				...
				else if ( _rtw_memcmp(pIE->data, ML_OUI, 4))		//Mirror
                {

					MirrorLink_info_handler( padapter, pIE );
                }
#endif				
				break;
